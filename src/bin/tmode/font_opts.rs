...
pub fn font_opts(matches: &ArgMatches) -> Result<(), Box<dyn Error>> {
    if matches.is_present("font_as_png") {
        font_as_png(
            ...
            matches.value_of("font_as_png").unwrap(),
            matches.value_of("font_width").unwrap(),
            matches.value_of("font_height"),
            matches.value_of("font_length").unwrap(),
            matches.value_of("chars_per_row").unwrap(),
        )?;
    }
    if matches.is_present("png_as_font") {
        png_as_font(
            matches.value_of("files").unwrap(),
            matches.value_of("png_as_font").unwrap(),
            matches.value_of("font_width").unwrap(),
            matches.value_of("font_height"),
            matches.value_of("font_length").unwrap(),
        )?;
    ...
    if matches.is_present("export_font") {
        export_font(
            matches.value_of("files").unwrap(),
            matches.value_of("export_font").unwrap(),
        )?;
    }
    if matches.is_present("export_font_as_png") {
        export_font_as_png(
            matches.value_of("files").unwrap(),
            matches.value_of("export_font_as_png").unwrap(),
            matches.value_of("chars_per_row").unwrap(),
        )?;
    }
    Ok(())
